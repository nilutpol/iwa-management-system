...
class Stock extends Component {
    ...
    getStockData = () => {
        let get_data = Axios.get(Constants.API_BASE_URL + 'get_stock', {
            params: {
    ...
    handleCloseNewStock = () => {
        this.setState({ openNewStock: false })
    }
    ...
    render() {
        const { is_fetching_data, data, metaData, yarn_list, openNewStock } = this.state;
        ...
        return (
            <React.Fragment>
                <Grid container spacing={1} justify="center" alignItems="center" style={{ backgroundColor: '#f7f9fc', padding: 10, marginBottom: 112 }}>
                    <Typography variant='h4' gutterBottom>Stock</Typography>
                    {/* <Grid item xs={12}>
                        <TextField
                            label="Search"
                            fullWidth
                    ...
                    <Grid item >
                        <Fab color="primary" className={classes.fab} onClick={() => this.setState({ openNewStock: true })}>
                            <AddIcon />
                        </Fab>
                    </Grid>
                </Grid>
                <Dialog open={openNewStock} onClose={this.handleCloseNewStock} disableBackdropClick maxWidth='md'>
                    ...
                    <DialogContent>
                        <Formik
                            ...
                            validate={(values) => {
                                ...
                                if (!this.pond_rc) {
                                    toast.error('Please select stock image.');
                                    errors.image = 'Please select stock image.';
                                }
                            ...
                            onSubmit={(values, { setSubmitting }) => {
                                ...
                                this.getImageData()
                                    .then((image_data) => {
                                        ...
                                        Axios.post(Constants.API_BASE_URL + 'add_stock', post_data)
                                            .then((res) => {
                                                setSubmitting(false);
                                                this.handleCloseNewStock();
                                                if (res.data.success) {
                                                    Swal.fire(
                                                        'Success',
                                                ...
                                                this.setState({ data: [] }, () => this.getStockData())
                                            })
                                            .catch(err => {
                                                setSubmitting(false);
                                                Swal.fire(
                                ...
                                return (<Form >
                                    <Grid container spacing={1}>
                                        <Grid item xs={12} md={12}>
                                            <Field name="stock_no" label="Stock No." margin="normal" component={FormikTextField} fullWidth variant='outlined' error={errors.stock_no !== undefined} helperText={errors.stock_no} />
                                        </Grid>
                                        {/* <Grid item xs={6} md={3}>
                                    ...
                                    <Button
                                        color="primary"
                                        onClick={this.handleCloseNewStock}
                                    >Cancel</Button>
                                    <Button
                                        variant="contained"
                                        color="primary"
...
export default withRouter(withStyles(styles, { withTheme: true })(Stock));